-- MISE V1 Schema
-- Component-based recipe model with 4-layer prompt architecture
...
create table public.users (
  ...
  tier            text not null default 'free'
                  check (tier in ('free', 'home_cook', 'creator', 'brigade')),
  stripe_customer_id text,
  default_complexity_mode text not null default 'kitchen'
                  check (default_complexity_mode in ('foundation', 'kitchen', 'riff')),
...
-----------------------------------------------------------
-- Fingerprints (operator-edited chef decision trees)
-----------------------------------------------------------
...
-----------------------------------------------------------
-- Chef Brains (compiled user personalization)
-----------------------------------------------------------
...
-----------------------------------------------------------
-- Recipes (component-based, structured JSON)
-----------------------------------------------------------
create table public.recipes (
  ...
  complexity_mode text not null default 'kitchen'
                  check (complexity_mode in ('foundation', 'kitchen', 'riff')),
...
-----------------------------------------------------------
-- Recipe Versions (with dial_direction tracking)
-----------------------------------------------------------
...
-----------------------------------------------------------
-- Recipe Forks (V3 UI, V1 data model readiness)
-----------------------------------------------------------
...
-----------------------------------------------------------
-- Generation Costs (operational reporting)
-----------------------------------------------------------
...
-----------------------------------------------------------
-- V2 Tables (created now for data model readiness)
-----------------------------------------------------------
...
create table public.preferences (
  ...
  source          text not null default 'explicit'
                  check (source in ('rating', 'dev_note', 'chat', 'tasting_note', 'explicit', 'inferred', 'pattern')),
  updated_at      timestamptz not null default now(),
  unique (user_id, key)
);
...
create table public.ideas (
  ...
  recipe_id       uuid references public.recipes on delete cascade,
  note            text,
  status          text not null default 'uncooked'
                  check (status in ('uncooked', 'annotated', 'generated')),
  created_at      timestamptz not null default now()
);
...
-----------------------------------------------------------
-- V3 Tables (data model readiness)
-----------------------------------------------------------
...
-- Recipes: own rows + public recipes readable by all
...
-- Recipe Versions: own recipe's versions
create policy "Versions read own" on public.recipe_versions for select
  using (exists (select 1 from public.recipes where recipes.id = recipe_versions.recipe_id and recipes.user_id = auth.uid()));
create policy "Versions write own" on public.recipe_versions for insert
  with check (exists (select 1 from public.recipes where recipes.id = recipe_versions.recipe_id and recipes.user_id = auth.uid()));

-- Fermentation: own rows
...
create trigger set_updated_at before update on public.fingerprints
  for each row execute function public.handle_updated_at();
create trigger set_updated_at before update on public.recipes
  for each row execute function public.handle_updated_at();
create trigger set_updated_at before update on public.fermentation_logs
  for each row execute function public.handle_updated_at();
create trigger set_updated_at before update on public.ai_provider_config
  for each row execute function public.handle_updated_at();
