        ...
        {showMore && (
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { v: constraints, s: setConstraints, p: 'Constraints' },
            ].map((f, i) => (
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          ...
          <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50 px-5 py-2.5">
            <div className="flex items-center gap-2 text-xs text-gray-500">
              {generating && <span className="inline-block h-2 w-2 animate-pulse rounded-full bg-green-500" />}
              {done ? '✓ Recipe ready' : 'Streaming…'}
            ...
            {done && (
              <div className="flex gap-1.5">
                ...
                <button onClick={() => { setContent(''); setDone(false); }} className="rounded-md bg-white px-2.5 py-1 text-xs text-gray-600 shadow-sm ring-1 ring-gray-200 hover:bg-gray-50">
                  New
                </button>
              </div>
            )}
